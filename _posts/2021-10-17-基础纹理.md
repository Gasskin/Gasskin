...
$M_{t2l}=\begin{bmatrix}t_{1} & b_{1} & n_{1}\\t_{2} & b_{2} & n_{2}\\t_{3} & b_{3} & n_{3}\\\end{bmatrix}\cdot\begin{bmatrix}x_{tangent}\\y_{tangent}\\z_{tangent}\\\end{bmatrix}=\begin{bmatrix}x_{local}\\y_{local}\\z_{local}\\\end{bmatrix}$

而从模型空间到切线空间的变换矩阵即上述矩阵的转置矩阵

$M_{l2t}=\begin{bmatrix}t_{1} & t_{2} & t_{3}\\b_{1} & b_{2} & b_{3}\\n_{1} & n_{2} & n_{3}\\\end{bmatrix}$
