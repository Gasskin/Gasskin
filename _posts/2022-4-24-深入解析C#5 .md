...
await后接着的表达式，必须是**可等待的**

假设一个返回类型为T的表达式，他需要是可等待的，那么T必须满足：

1. T必须具备（可以是扩展方法）无参方法GetAwaiter()，此方法返回一个**awaiter**类型
...
2. **awaiter**类型必须实现**INotifyCompletion**或者**ICriticalNotifyCompletion**接口

3. **awaiter**类型必须具有一个可读的实例属性IsCompleted，是bool

4. **awaiter**类型必须具有一个非泛型的无参方法GetResult()，**await关键字的返回值会和此方法保持一致**
...
结合以上信息，我们可以简单实现一个Awaiter

首先是等待对象

```c#
class TestAwaitable<TResult>
{
...
class TestAwaitable<TResult>
{
    ...
    public TestAwaiter<TResult> GetAwaiter() => new TestAwaiter<TResult>(this);

    // 1.继承接口
    public struct TestAwaiter<TResult> : INotifyCompletion
    {
        private readonly TestAwaitable<TResult> testAwaitable;

        // 2. 实现 IsCompleted 属性
        public bool IsCompleted => testAwaitable.IsCompleted;

        public TestAwaiter(TestAwaitable<TResult> testAwaitable)
        {
            this.testAwaitable = testAwaitable;
        }

        public void OnCompleted(Action continuation)
        {
            Console.WriteLine("TestAwaiter.OnCompleted");
            if (testAwaitable.AddFooContinuation(continuation))
            {
                Console.WriteLine("TestAwaiter.OnCompleted: 1");
            }
            else
            {
                Console.WriteLine("TestAwaiter.OnCompleted: 2");
                continuation();
            }
        }
...
class Program
{
    static async Task Main(string[] args)
    {
        var test = new TestAwaitable<string>();
...
![image-20230306165430168](https://raw.githubusercontent.com/Gasskin/CloudImg/master/img/202303061654213.png)
...
![image-20230306170227547](https://raw.githubusercontent.com/Gasskin/CloudImg/master/img/202303061702592.png)
